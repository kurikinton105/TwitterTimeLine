...
while(1): # データ取得の開始
  if diff.seconds >= 86400 or count >15: #24時間かループの制限に引っかかる時
    ...
    for i in range(len(get_data)):
      # create,"id",name,userid,text,["favorite_count"],["retweet_count"],"urls_link"
...
with open(path, mode='w') as f:
    ...
    f.write("<H1>"+str(date2 + datetime.timedelta(hours =9)) + "~" +str(date_create_at + datetime.timedelta(hours =9)) +"<H1>")
    for i in range(10):
